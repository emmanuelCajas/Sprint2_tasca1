-- MySQL Script generated by MySQL Workbench
-- Sat Mar 18 08:14:56 2023
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `mydb` ;

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mydb` DEFAULT CHARACTER SET utf8 ;
USE `mydb` ;

-- -----------------------------------------------------
-- Table `mydb`.`Provincia`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Provincia` (
  `id_provincia` INT NOT NULL,
  `nombre_provincia` VARCHAR(45) NULL,
  PRIMARY KEY (`id_provincia`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Localidad`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Localidad` (
  `id_localidad` INT NOT NULL,
  `nombre_localidad` VARCHAR(45) NULL,
  `id_provincia` INT NOT NULL,
  PRIMARY KEY (`id_localidad`),
  INDEX `fk_Localidad_Provincia_idx` (`id_provincia` ASC) ,
  CONSTRAINT `fk_Localidad_Provincia`
    FOREIGN KEY (`id_provincia`)
    REFERENCES `mydb`.`Provincia` (`id_provincia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Cliente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Cliente` (
  `id_cliente` INT NOT NULL,
  `nombre_cliente` VARCHAR(45) NULL,
  `apellido_cliente` VARCHAR(45) NULL,
  `direccion` VARCHAR(45) NULL,
  `codigo_postal` VARCHAR(45) NULL,
  `telefono` VARCHAR(9) NULL,
  `id_localidad` INT NOT NULL,
  PRIMARY KEY (`id_cliente`),
  INDEX `fk_Cliente_Localidad1_idx` (`id_localidad` ASC) ,
  CONSTRAINT `fk_Cliente_Localidad1`
    FOREIGN KEY (`id_localidad`)
    REFERENCES `mydb`.`Localidad` (`id_localidad`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Tienda`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Tienda` (
  `id_tienda` INT NOT NULL,
  `direccion` VARCHAR(45) NULL,
  `codigo_postal` VARCHAR(45) NULL,
  `id_localidad` INT NOT NULL,
  PRIMARY KEY (`id_tienda`),
  INDEX `fk_Tienda_Localidad1_idx` (`id_localidad` ASC) ,
  CONSTRAINT `fk_Tienda_Localidad1`
    FOREIGN KEY (`id_localidad`)
    REFERENCES `mydb`.`Localidad` (`id_localidad`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Empleado`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Empleado` (
  `id_empleado` INT NOT NULL,
  `nombre_empleado` VARCHAR(45) NULL,
  `apellido_empleado` VARCHAR(45) NULL,
  `nif` VARCHAR(45) NULL,
  `telefono` VARCHAR(9) NULL,
  `tipo_empleado` ENUM('cocinero', 'repartidor') NULL,
  `id_tienda` INT NOT NULL,
  PRIMARY KEY (`id_empleado`),
  INDEX `fk_Empleado_Tienda1_idx` (`id_tienda` ASC) ,
  CONSTRAINT `fk_Empleado_Tienda1`
    FOREIGN KEY (`id_tienda`)
    REFERENCES `mydb`.`Tienda` (`id_tienda`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Bebida`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Bebida` (
  `id_bebida` INT NOT NULL,
  `nombre_bebida` VARCHAR(45) NOT NULL,
  `descripcion_bebida` TEXT NOT NULL,
  `imagen_bebida` VARCHAR(45) NOT NULL,
  `precio_bebida` DOUBLE NOT NULL,
  PRIMARY KEY (`id_bebida`))
ENGINE = InnoDB;

INSERT INTO Bebida VALUES (1,'coca cola','gaseosa','imagen1',1.0);
INSERT INTO Bebida VALUES (1,'seven up','gaseosa','imagen1',1.0);
INSERT INTO Bebida VALUES (1,'aquarius','refresco','imagen1',1.0);
INSERT INTO Bebida VALUES (1,'agua','agua mineral','imagen1',1.0);


-- -----------------------------------------------------
-- Table `mydb`.`Hamburguesa`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Hamburguesa` (
  `id_producto` INT NOT NULL,
  `nombre_hamburguesa` VARCHAR(45) NOT NULL,
  `descripcion_hamburguesa` TEXT NOT NULL,
  `imagen_hamburguesa` VARCHAR(45) NOT NULL,
  `precio_hamburguesa` DOUBLE NOT NULL,
  PRIMARY KEY (`id_producto`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Categoria_pizza`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Categoria_pizza` (
  `id_categoria_pizza` INT NOT NULL,
  `nombre_categoria` VARCHAR(45) NULL,
  PRIMARY KEY (`id_categoria_pizza`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Pizza`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Pizza` (
  `id_pizza` INT NOT NULL,
  `nombre_pizza` VARCHAR(45) NOT NULL,
  `descripcion_pizza` TEXT NOT NULL,
  `imagen_pizza` VARCHAR(45) NOT NULL,
  `precio_pizza` DOUBLE NOT NULL,
  `id_categoria_pizza` INT NOT NULL,
  PRIMARY KEY (`id_pizza`),
  INDEX `fk_Producto_Categoria_pizza1_idx` (`id_categoria_pizza` ASC) ,
  CONSTRAINT `fk_Producto_Categoria_pizza1`
    FOREIGN KEY (`id_categoria_pizza`)
    REFERENCES `mydb`.`Categoria_pizza` (`id_categoria_pizza`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `mydb`.`Pedido`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`Pedido` (
  `id_pedidos` INT NOT NULL,
  `fecha_hora` DATETIME NULL,
  `tipo_pedido` ENUM('domicilio', 'recojo') NULL,
  `precio_total` DOUBLE NULL,
  `id_cliente` INT NOT NULL,
  `id_tienda` INT NOT NULL,
  `id_empleado` INT NULL,
  `fecha_hora_entrega` DATETIME NULL,
  `Bebida_id_bebida` INT NULL,
  `Hamburguesa_id_producto` INT NULL,
  `Pizza_id_pizza` INT NULL,
  `cant_pizza` SMALLINT NULL,
  `cant_bebida` SMALLINT NULL,
  `cant_hamburguesa` SMALLINT NULL,
  PRIMARY KEY (`id_pedidos`),
  INDEX `fk_Pedidos_Cliente1_idx` (`id_cliente` ASC) ,
  INDEX `fk_Pedidos_Tienda1_idx` (`id_tienda` ASC) ,
  INDEX `fk_Pedidos_Empleado1_idx` (`id_empleado` ASC) ,
  INDEX `fk_Pedido_Bebida1_idx` (`Bebida_id_bebida` ASC) ,
  INDEX `fk_Pedido_Hamburguesa1_idx` (`Hamburguesa_id_producto` ASC) ,
  INDEX `fk_Pedido_Pizza1_idx` (`Pizza_id_pizza` ASC) ,
  CONSTRAINT `fk_Pedidos_Cliente1`
    FOREIGN KEY (`id_cliente`)
    REFERENCES `mydb`.`Cliente` (`id_cliente`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Pedidos_Tienda1`
    FOREIGN KEY (`id_tienda`)
    REFERENCES `mydb`.`Tienda` (`id_tienda`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Pedidos_Empleado1`
    FOREIGN KEY (`id_empleado`)
    REFERENCES `mydb`.`Empleado` (`id_empleado`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Pedido_Bebida1`
    FOREIGN KEY (`Bebida_id_bebida`)
    REFERENCES `mydb`.`Bebida` (`id_bebida`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Pedido_Hamburguesa1`
    FOREIGN KEY (`Hamburguesa_id_producto`)
    REFERENCES `mydb`.`Hamburguesa` (`id_producto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Pedido_Pizza1`
    FOREIGN KEY (`Pizza_id_pizza`)
    REFERENCES `mydb`.`Pizza` (`id_pizza`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
